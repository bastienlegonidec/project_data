...
def process_images_to_csv(excel_path, images_folder, masks_folder, output_csv):
    ...
    df = pd.read_excel(excel_path, index_col=0, engine='openpyxl')

    # Main loop to extract features from each image and mask in the dataset
    for i in range(1, max(len(os.listdir(images_folder)), len(os.listdir(masks_folder))) + 1):
